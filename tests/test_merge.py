...
def test_MergeAssemble(sampleinfo, outdir, datadir):
	fni = MG.MergeInputNames(sampleinfo, 'Fev_merge_test', outdir=os.path.join(datadir,'merge'))
	...
	fna = MG.MergeAssemblyNames('Fev_merge_asm_test', outdir)
...
def test_MergeAssemblyNames(outdir):
	fn = MG.MergeAssemblyNames('test.man', outdir)
	assert fn._prefix == outdir+'/test.man'
	assert fn.ex_out('bed') == outdir+'/test.man.ex.bed.gz'
	assert fn.sj_out('txt') == outdir+'/test.man.sj.txt.gz'
	assert fn.ci_out() == outdir+'/test.man.ci.txt.gz'
	assert fn.genes_out('bed') == outdir+'/test.man.genes.bed.gz'
...
def test_MergeInputNames(testsampleinfo, outdir, datadir):
	...
	assert ('sample3', 'SJ/test3.sj.bed.gz') == fn.sjpaths()[2]
	assert ('sample4', os.path.join(datadir,'bedtools/test.bw')) == fn.bwpaths()[3]
