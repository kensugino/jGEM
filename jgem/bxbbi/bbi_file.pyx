...
cdef class BBIFile:
    ...
    cpdef visit_blocks_in_region( self, bits32 chrom_id, bits32 start, bits32 end, BlockHandler handler ):
        """
    ...
    cpdef summarize( self, bytes chrom, bits32 start, bits32 end, int summary_size ):
        """
        Gets `summary_size` data points over the regions `chrom`:`start`-`end`.
        """
        if start >= end:
            return None
        chrom_id, chrom_size = self.get_chrom_id_and_size( chrom )
        if chrom_id is None:
            return None
    ...
    cpdef summarize_from_full( self, bytes chrom, bits32 start, bits32 end, int summary_size ):
        """
        Gets `summary_size` data points over the regions `chrom`:`start`-`end`, 
    ...
    cpdef query( self, bytes chrom, bits32 start, bits32 end, int summary_size ):
        """
    ...
    cpdef get_chrom_id_and_size( self, bytes chrom ):
        """
