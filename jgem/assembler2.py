...
####### Generalized Suffix Tree & Gene Graph ###########################################

class GSTree(object):
    """Generalized Suffix Tree to find overlaps between paths. 
    Based on JHU Ben Langmead's example. 
    
    recs: [(pathid, [list of junction id]), ...] list of path id, junction ids pairs
    
    """
    
    class Node(object):
        def __init__(self, lab):
            self.lab = lab # label on path leading to this node # list of id's
            self.out = {}  # outgoing edges; maps id's to nodes
    ...
    def __init__(self, recs):
        ...
        Args:
            recs:[(path_id, [list of ids]), ...]
        """
        #s += '$'
        self.dic = dict(recs)
        self.s = s = []
        for k,v in recs:
            s += v+[-k] # negativ path id as delimiter
        #print(s)
    ...
    def find_connection(self, pathid):
        s = self.dic[pathid] # query
        cur = self.root
        i = 0
        found = [] # connected pathids
        while i < len(s):
            c = s[i]
            if c not in cur.out:
                return found
    ...
    def find_all_connections(self):
        revconn = {k: self.find_connection(k) for k in self.dic}
        conn = {}
        for k, v in revconn.items():
            for x in v:
                conn.setdefault(x,[]).append(k)
        return conn

def _printnode(node, s='', level=0):
    print('#'*level+' key:'+s+' lab:'+str(node.lab))
    for k,v in node.out.items():
        _printnode(v, str(k), level+1)
...
def chrom_assembler(bwpre, dstpre, genome, chrom, mingap=5e5, minbundlesize=10e6, np=2):
    bundles = find_bundles(bwpre, genome, [chrom], mingap, minbundlesize)
